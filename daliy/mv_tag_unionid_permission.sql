...
CREATE MATERIALIZED VIEW `mv_tag_unionid_permission_array` (
    `tag_name`,
    `union_id_array`
)
COMMENT "客户标签union_id数组"
DISTRIBUTED BY RANDOM
REFRESH ASYNC START("2026-03-31 06:00:00") EVERY(INTERVAL 1 DAY)
PROPERTIES (
    "replicated_storage" = "true",
...
SELECT
    tmp.tag_name,
    ARRAY_AGG(DISTINCT tmp.union_id) AS union_id_array
FROM (
    -- 重复添加
    SELECT union_id, '重复添加' AS tag_name
    FROM etl_db.tag_wxcust_profit
