...
def create_auth_cookie(username, modules, secret_key, max_age, cookie_name):
    """生成登录态 Cookie 值（已签名）。"""
    s = _serializer(secret_key)
    payload = {"u": username, "m": modules}
    return s.dumps(payload)


def verify_auth_cookie(cookie_value, secret_key, max_age, cookie_name):
    """校验并解析 Cookie，成功返回 (username, modules)，否则返回 None。"""
    if not cookie_value:
        return None
    s = _serializer(secret_key)
    try:
        data = s.loads(cookie_value, max_age=max_age)
        return (data.get("u"), data.get("m") or [])
    except Exception:
        return None
...
def create_portal_token(username, modules, secret_key, max_age_seconds=PORTAL_TOKEN_MAX_AGE):
...
def verify_portal_token(token, secret_key, max_age_seconds=PORTAL_TOKEN_MAX_AGE):
    """校验 portal_token，成功返回 (username, modules)，否则返回 None。"""
